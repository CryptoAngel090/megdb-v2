...
interface MediaCardProps {
  ...
  /** Route prefetch strategy for this card link. @default false */
...
export function MediaCard({
  ...
  const onPosterLoad = useCallback(
    (event: SyntheticEvent<HTMLImageElement>) => {
      onPosterLoadingComplete(event.currentTarget)
    },
    [onPosterLoadingComplete]
  )
  ...
  const inner = (
    <Link
      ...
      className={`${cardClassWithBadges} ${styles.mediaCard} ${styles.mediaCardSubgrid} ${CARD_SIZE_MOD[cardSize]} card-hover hover-lift-card`}
      data-tmdb-id={id}
  ...
  if (prefersReducedMotion) {
    return <div className={`${styles.cardTapWrapper} media-card-wrapper`}>{inner}</div>
  }
